    ...
    async function processFile(fileHandle: any, basePath: string, fileName: string): Promise<void> {
      try {
        ...
        if (existing && existing.url) {
          // Verify if the cached base64 data exists
          const cachedBase64 = await dbManager.getCache(`base64-${existing.id}`);
          if (cachedBase64) {
        ...
        await dbManager.setCache(`base64-${track.id}`, base64Data);
...
export const getBlobUrlForTrack = async (trackId: string): Promise<string | null> => {
  try {
    return await dbManager.getCache(`base64-${trackId}`);
  } catch {
    return null;
  }
};